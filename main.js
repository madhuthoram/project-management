                ...
                for (var j = 0; j<items.length; j++) {
                    if(items[j] == ";") {
                        if(semi1 == -1) {
                            ...
                            content += "></input><div></div> <input type='text' id='t" + taskCount + "i" + itemCount + "' onchange='changeItem(" + num + ", " + taskCount + ", " + itemCount +")' value='" + inners +"'></input></label>1</div>"
                            semi1 = semi2;
                        }
                    }
                }
...
function newTask(num) {
    ...
    for(var i=1; i< projectNum.length;i++) {
     ...
     localStorage.setItem("project" + num, projectNum + "New Tasks;");
     loadContent(num);
}
